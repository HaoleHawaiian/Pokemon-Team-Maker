...
## The App
Running on Streamlit locally, the GloVe and BERT models took several minutes, leading to a poorer user experience, so for the current version of the app, I have removed those options. However, if you want to take the juptyer notebook and enter in your own inputs there, it runs much faster than streamlit, even with the GloVe and BERT options. Further next steps are to allow users to select a team they think is a better fit for their personality and after gathering some data, will only present the crowd favorite. Each pokemon recommendation includes a link that takes you to a page containing the Pokedex entries to compare to your inputs.
